...
### F-10 · `subscription_repair_2026_08_14` is an orphan table in production — `LOW`

A one-off repair table left behind in the production database. 2 rows
(`user_id`, `email`, `old_end_at`, `new_end_at`, `days_added`, `repaired_at`).
No model declares it, no migration creates it, nothing in the codebase reads it.

It is a record of a subscription repair someone ran by hand on 2026-08-14, and
it is the only surviving evidence of what that repair did — so it is *data*, not
dead code, and `ghawy_baseline` deliberately does not create it. A clean
database will not have it and does not need it.

**Not acted on.** Deleting it destroys an audit trail; keeping it costs nothing.
Decide in Phase 5 whether to archive the two rows and drop the table.

### F-11 · Three constraint/index names differ between production and the models — `COSMETIC`
...
### F-12 · Six revisions are completely empty — `INFORMATIONAL`

`2334af3cafbd` (daily_reports), `234cce29f5f4` (ai_updates), `307efdf1db45`
(notifications), `920b33de7a7e` (suggested_guests), `a50de1dd7efd` (feedbacks)
and the merge `4823c6c0b288`. The first five were autogenerated against a
database `create_all` had already updated, so alembic correctly found nothing to
do and wrote `pass` — the tables in their titles were never created by them.

They are kept: deleting a revision rewrites history for every database stamped
past it, and they cost one no-op each. `ghawy_baseline` is what actually creates
those five tables now.

### F-13 · `# Force Reload` / `# reload` left at the bottom of `main.py` — `TRIVIAL`

Two stray comments at the end of the file, left over from a deploy that needed
the file to change. Harmless. Left for the Phase 5 comment pass rather than
touched here.
